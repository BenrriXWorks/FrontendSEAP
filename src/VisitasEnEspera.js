...
const AppScreen = ({ navigation, route }) => {
    ...
    const eliminarVisitaSQLite = (visita) => {
      ...
      let id = visita.ID;
      clientes = clientes.filter((cliente) => { return (id !== cliente.ID) });
      /* ----------------------------------------------------------- */
    ...
    const handleSubirVisitas = () => {
      ...
      if (!clientes.length){
        Alert.alert("Error", "No hay visitas para subir, volviendo al menu principal.");
        navigation.goBack();
      }
      else
        navigation.navigate("ConfirmScreen", { state:"upload" });
    }
    ...
    const handleFlechaPresionada = (visita) => {
      ...
      Alert.alert(
        ...
        [
          ...
          { text: "Confirmar", onPress: () => { eliminarVisitaSQLite(visita) } }
        ]
      );
    };
      ...
      <TouchableWithoutFeedback onPress={() => setModalVisible(false)}>
        <View style={{ flex: 1, backgroundColor: '#1069B4' }}>
          <View style={{ backgroundColor: '#00A651', justifyContent: 'flex-end', alignItems: 'center', height: '15%' }}>
            <Text style={{ color: 'white', fontSize: 18, marginBottom: 15, fontWeight: 'bold' }}>VISITAS EN ESPERA DE SUBIDA</Text>
          </View>
          <View style={{ flex: 1, paddingHorizontal: 20, paddingTop: 10 }}>
            ...
            >
                <ScrollView id="ScrollClientes" style={{ backgroundColor: 'transparent', borderRadius: 15, padding: 10 }}>
                    {clientes.map((cliente, index) => {
                        const tintColor = index % 2 === tintColorIndex ? '#BEBD88' : '#88807B'; // Alterna entre los dos colores
                        return (
                        {/* Items de visita */},
                        <TouchableOpacity
                            key={index}
              ...
              <TouchableWithoutFeedback onPress={() => setModalVisible(false)}>
                <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', backgroundColor: 'rgba(0, 0, 0, 0.5)' }}>
                  <View style={{ backgroundColor: 'white', padding: 20, borderRadius: 10, width: '90%' }}>
                    <Text style={{ fontSize: 20, fontWeight: 'bold', marginBottom: 10 }}>Detalles de visita:</Text>
            ...
            <View style={{ alignItems: 'center' }}>
              {/* Boton Subir visitas */}
              <TouchableOpacity style={{ backgroundColor: '#00A651', width: '80%', borderRadius: 20, padding: 10, marginTop: 15, alignItems: 'center' }} onPress={handleSubirVisitas}>
                <Text style={{ color: 'white', fontSize: 18, fontWeight: 'bold' }}>SUBIR VISITAS</Text>
              </TouchableOpacity>
               {/* Boton volver */}
              <TouchableOpacity style={{ backgroundColor: '#EA3B44', width: '40%', borderRadius: 20, marginBottom:-30, padding: 10, marginTop: 15, alignItems: 'center' }} onPress={navigation.goBack}>
