...
const RutInput = ({rut, setRut}) => {
  ...
  function formatRut(input) {
    ...
    let cleanRut = input.toUpperCase().replace(/[^0-9K.-]$/gi, '').replace(/[.-]/g, '');
    
    // Borra lo que haya despues de la k si hay una k
    let posK = cleanRut.search('K') + 1;
    if (posK)
      cleanRut = cleanRut.substring(0, posK);
    ...
    if (cleanRut.length === 8 || cleanRut.length === 9) {
      ...
      if (cleanRut.length === 9)
        displacenment = displacenment.map((n)=>{return n+1});
...
function mod11(rutStr){
  ...
  let [numerosStr, digitoVerificadorStr] = rutStr.replaceAll(".","").split("-");

  // Realizar la reduccion
  let factor = 2, value = 0;
...
export default function LoginScreen({ navigation }) {
  ...
  const handleLogin = async () => {
    ...
    if (query.success) {
      // Guardar las credenciales en local si rememberMe es verdadero
      if (rememberMe) {
        try {
    ...
    <KeyboardAvoidingView style={styles.container} behavior="padding">
      ...
      <View style={styles.inputContainer}>
        ...
        <Text style={styles.forgotPasswordText} 
          onPress={()=>{
            Alert.alert("TO-DO:", "Modal de reestablecer contraseña poniendo el correo")}
        }>
